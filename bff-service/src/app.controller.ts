...
export class AppController {
  ...
  async proxy<T>(@Req() req: Request, @Res() res: Response): Promise<void> {
    ...
    const { serviceName, servicePath, url } = this.discoveryService.discoverEndpoint(req.path);
    const body = req.body;
    ...
    const cacheParameters = this.getCacheParameters(serviceName, servicePath, method);
    if (cacheParameters) {
    ...
    if (cacheParameters) {
      if (cacheParameters.statuses.includes(status)) {
        await this.cacheManager.set(cacheParameters.key, data, { ttl: cacheParameters.ttl });
      }
    }
